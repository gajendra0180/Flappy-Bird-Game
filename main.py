import random
import sys  # we wwill use sys.exit to exit the program
import pygame
...
def mainGame():
    ...
    # creating 2 pipers for blitting on the screen
    ...
    while True:
        ...
        for pipe in upperPipes:
            ...
            print(pipeMidPos, playerMidPos, pipeMidPos)
...
# Add a new pipe when a  first pipe is about to cross the part of the screen
        ...
        # SCREEN.blit(GAME_SPRITES['base'], (basex, playery))
        SCREEN.blit(GAME_SPRITES['player'], (playerx, playery))

        myDigits = [int(x) for x in list(str(score))]
        width = 0
        # print("type", type(GAME_SPRITES['numbers']))
...
def isCollide(playerx, playery, upperPipes, lowerPipes):
    if playery > GROUNDY-45 or playery < 0:
        SCREEN.blit(GAME_SPRITES['game_over'], (0, SCREENWIDTH/2))
        pygame.display.update()
        FPSCLOCK.tick(FPS)
        return True
    for pipe in upperPipes:
...
def getRandomPipe():
    # generate positions of two pipes for blitting on the screen(one straight and one top rotated pipe)
    pipeHeight = GAME_SPRITES['pipe'][0].get_height()
...
if __name__ == "__main__":
    # This will be our main function from where our game will start
    pygame.init()  # initialises all pygame modules
    FPSCLOCK = pygame.time.Clock()  # controls FPS of game
    pygame.display.set_caption('Flappy Bird game by gajendra_0180')
    ...
    GAME_SPRITES['game_over'] = pygame.image.load(
    ...
    while True:  # Game loop
        welcomeScreen()  # shows welcome screen to user untial a choice is made
